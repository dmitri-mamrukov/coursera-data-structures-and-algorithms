...
class UkkonenSuffixTree():
    ...
    def __repr__(self):
        ...
        for edge in edges:
            ...
            top = min(self.n, edge.last_char_index)
...
class Solver:
    ...
    def find_shortest_nonshared_substring(self, text1, text2):
        ...
        tree = UkkonenSuffixTree(text2)

        for k in range(1, len(text1) + 1):
            shared = True
            for kmer in self.generate_kmers(text1, k):
                if not tree.has_substring(kmer):
                    shared = False
                    break
    ...
    def generate_kmers(self, sequence, k):
        """
        Generates all the possible substrings of length k that are contained
        in the string sequence.

        The term k-mer typically refers to all the possible substrings of
        length k that are contained in a string.
        """
